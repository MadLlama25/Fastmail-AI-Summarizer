// secure-storage.js - Shared secure storage utilities for Firefox Extension
...
class SecureStorage {
  ...
  getSalt() {
    const extensionId = browser.runtime.id || 'fastmail-email-summarizer';
...
async function encodeToken(token) {
  ...
  const password = (browser.runtime.id || 'fastmail-email-summarizer') + '_v2_secure';
